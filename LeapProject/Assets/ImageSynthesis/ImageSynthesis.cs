...
public class ImageSynthesis : MonoBehaviour {
	...
	void Start()
	{
		...
		capturePasses[1].camera = Camera.allCameras[1];
		capturePasses[2].camera = Camera.allCameras[2];
		capturePasses[3].camera = Camera.allCameras[3];		
		capturePasses[4].camera = CreateHiddenCamera (capturePasses[4].name);		

		// for (int q = 4; q < capturePasses.Length; q++)
	...
	public void OnCameraChange()
	{
		...
		var cam1 = Camera.allCameras[1];
		var cam2 = Camera.allCameras[2];
		...
		// {
		// 	if (pass.camera == mainCamera || pass.camera == cam1 || pass.camera == cam2 || pass.camera == cam3)
		// 		continue;
		...
		// SetupCameraWithReplacementShader(capturePasses[4].camera, uberReplacementShader, ReplacelementModes.ObjectId);
		// SetupCameraWithReplacementShader(capturePasses[5].camera, uberReplacementShader, ReplacelementModes.CatergoryId);
		SetupCameraWithReplacementShader(capturePasses[4].camera, uberReplacementShader, ReplacelementModes.DepthCompressed, Color.white);
		// SetupCameraWithReplacementShader(capturePasses[7].camera, uberReplacementShader, ReplacelementModes.Normals);
		// SetupCameraWithPostShader(capturePasses[8].camera, opticalFlowMaterial, DepthTextureMode.Depth | DepthTextureMode.MotionVectors);
	}
